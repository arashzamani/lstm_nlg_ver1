...
class StructureModel:
    ...
    def word_model(cls, structure, seq_length, word2vec, word_list, vocabulary, vocab_length, file=None):
        ...
        y = numpy.reshape(dataY, (n_patterns, 1))
        ...
        word_model.add(Dense(vocab_length, activation='softmax'))

        print word_model.summary()

        # word_model.load_weights('model_for_hafez.h5')

        word_model.compile(loss='sparse_categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
    ...
    def load_test_data(cls, seq_length, word_to_int):
        test_data_file = UFile(os.getcwd() + '/test_hafez.txt')
